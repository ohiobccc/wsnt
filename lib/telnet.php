<?php
/* Telnet Client Processor 
 *
 * Receives XHR and issues telnet commands to modem
 *
 * @author	Tom Kisha
 * @version	0.1
...
// Generates windstream support password
function generateSessionPassword($pre, $mac) {
	$tmp = strtolower($mac);
    $out = $pre . substr($tmp, 6, 6);
    return $out;
}

// Splits POST to array for loop processing
function parseCommandList ($list) {
	$cmd = html_entity_decode($list);
	$out = preg_split ("/\n/", $cmd);
	return $out;
}

// Send commands to telnet client and prettify output
function executeSessionCommands ($mdm) {
	$out = '<h3>Session Begin...</h3>';
	$tt = new Telnet ($mdm['ip'], $mdm['port'], 10, 'Router');
	$tt->login($mdm['un'], $mdm['pw']);
	$tt->setPrompt("\n >");
	for ($x = 0; $x < count($mdm['commands']); $x++) {
		$out .= '<hr />';
		$out .= '<div style="font-size:110%;"><b>Command</b>:'.$mdm['commands'][$x].'</div>';
		if ($mdm['commands'][$x] == 'sh') {
			$tt->setPrompt('#');
		}
		$commandOutput = $tt->exec($mdm['commands'][$x]);
		sleep(3);
		$out .= '<pre>';
		$out .= $commandOutput;
		$out .= '</pre>';
	...
	$out .= '<h3>Session End...</h3>';
	//echo $out;
	$out .= '<hr /><h3>Global command buffer....</h3><pre>';
	$out .= '<sub style="font-size:7pt;">'.$tt->getGlobalBuffer().'</sub>';
	$out .= '</pre>';
	$tt->disconnect();
	return $out;
}
...
// Make sure request is sumitted via POST, grab data, then send to functions
if ($_POST) {
    if (isset($_POST['mac']) && isset($_POST['ip'])) {
		$mdm['ip'] = $_POST['ip'];
		$mdm['mac'] = $_POST['mac'];
		$mdm['port'] = $_POST['port'];
		$mdm['pw'] = generateSessionPassword ('###WINDSTREAMPROPRIETARY###', $mdm['mac']);
		$mdm['commands'] = parseCommandList($_POST['commands']);
		
        $out = executeSessionCommands ($mdm);
		// Output whatever comes back, for better or worse
        echo $out;
    }
}
...
$tt->exec('wlctl scan');
sleep(4);
$out = $tt->exec('wlctl scanresults');
$dsl = $tt->exec('xdslctl info --show');


echo '<h2>Wireless Site Survey</h2>';
echo '<pre>'.$out.'</pre>';
echo '<h2>DSL signal data</h2>';
echo '<input type="button" value="Send to Auditor" />';
echo '<pre>'.$dsl.'</pre>';


$tt->setPrompt('>');
$tt->waitPrompt();
$tt->write('wlctl scanresults');
$tt->setPrompt('>');
$tt->waitPrompt();
*/
?>
